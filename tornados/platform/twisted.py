...
from tornados.concurrent import Future
from tornados.escape import utf8
from tornados import gen
import tornados.ioloop
from tornados.log import app_log
from tornados.netutil import Resolver
from tornados.stack_context import NullContext, wrap
from tornados.ioloop import IOLoop
from tornados.util import timedelta_to_seconds
...
class TornadoReactor(PosixReactorBase):
    ...
    def __init__(self, io_loop=None):
        if not io_loop:
            io_loop = tornados.ioloop.IOLoop.current()
...
class _FD(object):
    ...
    def doRead(self):
        if not self.lost:
            self.handler(self.fileobj, tornados.ioloop.IOLoop.READ)

    def doWrite(self):
        if not self.lost:
            self.handler(self.fileobj, tornados.ioloop.IOLoop.WRITE)

    def connectionLost(self, reason):
        if not self.lost:
            self.handler(self.fileobj, tornados.ioloop.IOLoop.ERROR)
            self.lost = True
...
class TwistedIOLoop(tornados.ioloop.IOLoop):
    ...
    def add_handler(self, fd, handler, events):
        ...
        if events & tornados.ioloop.IOLoop.READ:
            self.fds[fd].reading = True
            self.reactor.addReader(self.fds[fd])
        if events & tornados.ioloop.IOLoop.WRITE:
...
class TwistedResolver(Resolver):
    ...
    def initialize(self, io_loop=None):
        ...
        self.reactor = tornados.platform.twisted.TornadoReactor(io_loop)
